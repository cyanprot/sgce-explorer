...
- **Wild-type vs Mutant** toggle — full 437 aa structure vs truncated 67 aa fragment with STOP marker
...
```mermaid
graph TD
    A[main.tsx] --> B[App.tsx — 5 Tab Router]
    B --> C[ProteinStructure3D]
    B --> V[VariantsPanel — lollipop + filters + detail]
    B --> D[CentralDogma — Orchestrator]
    B --> E[ImprintingPanel]
    B --> R[ResearchPanel — Orchestrator]
    B --> S[variantStore — selected variant drives all tabs]
...
366 tests across 40 test files using **Vitest** + **React Testing Library**.
